    ...
    abrirModalVaga() {
      const config = new MatDialogConfig();
        const titulo = `Editar Vaga`;
          ...
          config.data = { titulo };
      const dialogRef = this.dialog.open(EmpresaModalComponent, config);
  ...
  login($event: Event) {
    ...
    this.authService.login(dto).subscribe({
      ...
      error: function (err) {
        ...
        var mensagem = 'Credenciais inválidas. Caso já tenha feito o cadastro verifique seu e-mail cadastrado';
        var tentativas = null;
